...
public class DeepHedging {
	 ...
	 * MultiLayerConfiguration, which we have seen in the bubble example. Here we need it because our network structure is
	 * more complex: for example, the loss function depends not only on the final choice but on all the strategies in time.
	 * Basically, this neural network will be made of (numberOfTimes-1) sub-neural networks, each of which is responsible to
	 * compute the strategy at the given time. These networks have two hidden layers with number of nodes (same for each network)
	...
	private void constructNetwork() {
		 ...
		 * - layers: they allow to go forward and, in this case, to find the strategies once their parameters
		 * are trained
		 * - vertices, not to be confused with nodes: vertices are used to give new inputs (for example, the new price at
		...
		String stringForPrice = "price0"; 
		String stringForFirstHiddenLayer = "firstHiddenLayer0";
		String stringForSecondHiddenLayer = "secondHiddenLayer0";

		//"strategy" will always mean how much we buy or sell
		String stringForStrategy = "strategy0" + 0;

		/*
		 * "strategyProfitAndLoss" will always mean how much we gain/lose from the single strategy decided at time t_n, when we see
		 * the new price at time t_{n+1}
		 */
		String stringForStrategyProfitAndLoss;


		//The cumulative strategy profit and loss at given time will be equal to the sum of all strategyProfitAndLosses up to that time..
		String stringForOldCumulativeStrategyProfitAndLoss = "cumulativeStrategyProfitAndLoss0";

		//..so: the new one will be the old one plus strategyProfitAndLoss
		String stringForCumulativeStrategyProfitAndLoss;
		 ...
		 * train and test the network, we will give an input identified by the string contained in inputName.
		...
		/*
		 * The first input is quite dummy: it will be just the starting value of profits and losses coming from the strategy,
		 * i.e., zero. This value (zero) will be specified later. 
		 */
		builder.addInputs(stringForOldCumulativeStrategyProfitAndLoss)
		//the second one is the initial price of the underlying (it will be also specified later)
		...
		//the number of inputs must be equal to the number of outputs of the layer above, otherwise we get an exception
		 ...
		 * Note that the vertex created later by addVertex(stringForStrategyProfitAndLoss..) depends on this (partial) output
		 */
		...
		//we need it to compute the difference with respect to the next price
		String stringForOldPrice = "price0";

		/*
		 * Now we proceed with a for loop which runs over time in order to construct the rest of the sub-networks.
		 * Each sub-network will provide the optimal strategy at the specific time.
		 * In the meantime, we also have to take care of updating the cumulative profit and loss of the strategy, because
		 * this will contribute to the loss function which we need to train the network      
		 */
		for(int timeIndex = 1; timeIndex < numberOfTimes - 1; timeIndex ++) {

			/*
			 * All these strings identify the specific layer and vertices. Their name must be always different,
			 * which we achieve by placing the specific iteration number at the end
			 */
			...
			stringForStrategyProfitAndLoss = "strategyProfitAndLoss" + timeIndex;
			stringForCumulativeStrategyProfitAndLoss  = "cumulativeStrategyProfitAndLoss" + timeIndex;
			 ...
			 * on the vertex to get the value of this vertex, and strings which identify the vertices on which the operation must be
			 * performed. So: in this case, based on the price, we compute the difference between new price and last price..
			 */
			.addVertex(stringForIncrement, new ElementWiseVertex(Op.Subtract), stringForPrice, stringForOldPrice)
			//..and based on this difference, the profit and loss of the last computed strategy..
			.addVertex(stringForStrategyProfitAndLoss, new ElementWiseVertex(Op.Product), stringForIncrement, stringForStrategy)
			//..which we then sum to the vertex identified by stringForOldCumulativeStrategyProfitAndLoss, to compute the new value
			.addVertex(stringForCumulativeStrategyProfitAndLoss, new ElementWiseVertex(Op.Add), stringForStrategyProfitAndLoss, stringForOldCumulativeStrategyProfitAndLoss)
			//now we give the new price as an input to the first layer of the new sub-network
			.addLayer(stringForFirstHiddenLayer, new DenseLayer.Builder().nIn(1).nOut(numberOfNodesForFirstLayer)
					.activation(new ActivationReLU()).build(), stringForPrice)
			...
			//..and at a node identified by this string, we place the output of the second layer: it is the value of the optimal strategy at this time step
			...
			//finally, we update the strings for the old price and the old cumulative profit and loss of the strategy
			stringForOldPrice = "price" + timeIndex;
			stringForOldCumulativeStrategyProfitAndLoss  = "cumulativeStrategyProfitAndLoss" + timeIndex;
		}
		...
		//and based on this, as we did in the for loop, we compute increment, profit and loss for the last strategy and cumulative profit and loss
		.addVertex("lastIncrement", new ElementWiseVertex(Op.Subtract), "lastPrice", stringForPrice)
		.addVertex("lastStrategyProfitAndLoss", new ElementWiseVertex(Op.Product), "lastIncrement", stringForStrategy)
		.addVertex("lastCumulativeStrategyProfitAndLoss", new ElementWiseVertex(Op.Add), "lastStrategyProfitAndLoss", stringForOldCumulativeStrategyProfitAndLoss)
		...
		//this is the cumulative profit and loss of the strategy minus the payoff..
		.addVertex("strategyMinusPayoff",  new ElementWiseVertex(Op.Subtract), "lastCumulativeStrategyProfitAndLoss", "payoff")		
		...
		/*
		 * This is now the output of our network, based on which we compute the loss function.
		 * Basically we want this output to have the same value as finalPortfolioValue, but to be defined as a
		 * layer because in this way we can specify the loss function. It has one input and one output.
		 * Later, we force it its parameters to be one and zero, respectively, so that it has the value
	...
	private void trainNetwork() throws CalculationException {
		...
		/*
		 * The length of this array must be equal to the number of inputs that we give to the network constructed
		 * in the method above: we had one input at the beginning (equal to zero, for the initial value of the
		 * cumulative sum of the strategies' profits and losses), numberOfTimes prices and then the other dummy
		 * input equal to zero at the end. For each of these inputs, we have to give a number of values equal to
		 * numberOfSimulations
		 * 
		 */
		 ...
		 * arrays
		 */
		double[][] initialProfitAndLossStrategiesTrain = new double[numberOfSimulations][1];

		inputsArrayTrain[0] = new NDArray(initialProfitAndLossStrategiesTrain); 
		...
		for(int timeIndex = 0; timeIndex < numberOfTimes; timeIndex ++) {
			...
			//they are matrices of one column: the constructor of NDArray accepts matrices, not one-dimensional arrays
		...
		//dummy matrix, same thing as for initialProfitAndLossStrategiesTrain
		 ...
		 */
		double[][] finalProfitAndLossesTrain = new double[numberOfSimulations][1];

		//we express them in terms of NDArray..
		INDArray[] outputsArrayTrain = new INDArray[1];

		outputsArrayTrain[0] = new NDArray(finalProfitAndLossesTrain);		
	...
	public double[] getPortfolioValuesForTesting(int numberOfSimulationsForTesting, int seedForTesting) throws CalculationException {
		...
		double[][] initialProfitAndLossStrategiesTest = new double[numberOfSimulationsForTesting][1];

		inputsArrayTest[0] = new NDArray(initialProfitAndLossStrategiesTest); 
