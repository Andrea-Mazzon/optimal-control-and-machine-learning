...
public class PolicyImprovement {
	...
	//the interval where the state process can take values. Again, for our algorithm it has to be bounded
	...
	/**
	 * It constructs an object to solve a general stochastic optimal control problem in continuous time, with one-dimensional domain for the controlled process.
	 ...
	 * @param finalTime, the time at which the final reward is given
	 * @param timeStep, the discretization step of the time interval [0, finalTime]
	 * @param requiredPrecision, the iterations stop if the norm of the difference between the new and past values of the matrix
	...
	private void computeSolutionAndOptimalControl() throws Exception {
		...
		while (differenceNorm > requiredPrecision & iterationCounter < maxNumberIterations) {
			...
			oldSolution = updatedValueFunction.clone();
	...
	/*
	 * At the k-th iteration of the Policy Improvement Algorithm, this method returns a matrix whose element of row i and column j
	...
	private double[][] getMaximizingControl(double[][] currentValueFunction){
		...
		for (int timeIndex = 1; timeIndex <= numberOfTimeSteps; timeIndex ++) {
			...
			for (int spaceIndex = 0; spaceIndex <= numberOfSpaceSteps; spaceIndex ++) {
				...
				double firstSpaceDerivative = computeFirstDerivative(updatedValueFunction, timeIndex, spaceIndex);
				double secondSpaceDerivative = computeSecondDerivative(updatedValueFunction, timeIndex, spaceIndex);
				double[] valuesForControls = new double[numberOfControls];
				
				//we compute the values for all controls..
	...
	/*
	 * This method computes the approximated first derivative of functionAsMatrix at the point in time and space
	 * determined by timeIndex and spaceIndex. In the method getMaximizingControl, it gets called passing
	 * updatedValueFunction
	 */
	private double computeFirstDerivative(double[][] functionAsMatrix, int timeIndex, int spaceIndex) {

		if (spaceIndex == 0) {
			return (functionAsMatrix[timeIndex][1]-functionAsMatrix[timeIndex][0])/spaceStep;
		} else if (spaceIndex == numberOfSpaceSteps) {
			return (functionAsMatrix[timeIndex][numberOfSpaceSteps]-functionAsMatrix[timeIndex][numberOfSpaceSteps - 1])/spaceStep;
		}
		else {
			return (functionAsMatrix[timeIndex][spaceIndex + 1]-functionAsMatrix[timeIndex ][spaceIndex - 1])/(2*spaceStep);
		}
	}
	
	/*
	 * This method computes the approximated second derivative of functionAsMatrix at the point in time and space
	 * determined by timeIndex and spaceIndex. In the method getMaximizingControl, it gets called passing
	 * updatedValueFunction
	 */
	private double computeSecondDerivative(double[][] functionAsMatrix, int timeIndex, int spaceIndex) {

		if (spaceIndex == 0) {
			return (functionAsMatrix[timeIndex][2]-2*functionAsMatrix[timeIndex][1]+functionAsMatrix[timeIndex][0])/(spaceStep*spaceStep);

		} else if (spaceIndex == numberOfSpaceSteps) {
			return (functionAsMatrix[timeIndex][numberOfSpaceSteps]-2*functionAsMatrix[timeIndex][numberOfSpaceSteps - 1]
					+functionAsMatrix[timeIndex][numberOfSpaceSteps - 2])/(spaceStep*spaceStep);
		}
		else {
			return (functionAsMatrix[timeIndex][spaceIndex + 1]-2*functionAsMatrix[timeIndex][spaceIndex]
					+functionAsMatrix[timeIndex][spaceIndex - 1])/(spaceStep*spaceStep);
		}
	}
